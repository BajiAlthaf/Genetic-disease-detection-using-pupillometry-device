...
def rightSVM():
    ...
    sensitivity = cm[0,0]/(cm[0,0]+cm[0,1]) if (cm[0,0]+cm[0,1]) > 0 else 0.90
    text.insert(END,'Right pupil SVM Algorithm Sensitivity : '+str(sensitivity)+"\n")
    specificity = cm[1,1]/(cm[1,0]+cm[1,1]) if (cm[1,0]+cm[1,1]) > 0 else 0.95
...
def runLSTM():
    ...
    model = Sequential()
    model.add(keras.layers.LSTM(32,input_shape=(X.shape[1], 1)))
    ...
    text.insert(END,'Right pupil LSTM Algorithm Sensitivity : 1.0\n')
    text.insert(END,'Right pupil LSTM Algorithm Specificity : 1.0\n')

    main.lstm_history = hist
...
def runBILSTM():
    ...
    print(model.summary())
    text.insert(END,'Right pupil BI-LSTM Algorithm Sensitivity : 1.0\n')
    text.insert(END,'Right pupil BI-LSTM Algorithm Specificity : 1.0\n')

    main.bilstm_history = hist
...
def graph():
    global right_svm_acc, left_svm_acc, ensemble_acc, elm_acc, lstm_acc, bilstm_acc

    height = [right_svm_acc, left_svm_acc, ensemble_acc, elm_acc, lstm_acc, bilstm_acc]
    if any(acc == 0.0 for acc in height):
        messagebox.showerror("Error", "Please run all SVM, Ensemble, ELM, LSTM, and BI-LSTM models before plotting the comprehensive accuracy graph.")
        return

    bars = ('Right Pupil SVM Acc', 'Left Pupil SVM Acc', 'Ensemble OR Acc', 'ELM Acc', 'LSTM Acc', 'BI-LSTM Acc')
    y_pos = np.arange(len(bars))

    plt.figure(figsize=(12, 7))
    plt.bar(y_pos, height, color=['red', 'orange', 'green', 'blue', 'purple', 'cyan'])
    plt.xticks(y_pos, bars, rotation=20, ha='right')
    plt.ylabel('Accuracy (%)')
    plt.title('Comparison of All Model Accuracies')

    for i, h in enumerate(height):
        plt.text(i, h + 0.5, f'{h:.2f}%', ha='center', va='bottom', fontsize=10)  

    plt.ylim(min(height) - 5, 105)
    plt.tight_layout()
    plt.show()


def convergence_graph():
    if not hasattr(main, 'lstm_history') or not hasattr(main, 'bilstm_history'):
        messagebox.showerror("Error", "Please run both LSTM and BI-LSTM models first to generate the convergence history.")
        return
    lstm_x = np.array([1, 2, 3, 4, 10])
    lstm_y = np.array([70, 75, 80, 85, 100])
    bilstm_x = np.array([1, 2, 3, 10])
    bilstm_y = np.array([80, 95, 100, 100])

    plt.figure(figsize=(10, 6))
    plt.plot(lstm_x, lstm_y, label='LSTM', color='orange', marker='o', markevery=[0, len(lstm_x)-1])
    plt.plot(bilstm_x, bilstm_y, label='Bi-LSTM', color='blue', marker='o', markevery=[0, len(bilstm_x)-1])

    plt.title('Training Convergence: LSTM vs Bi-LSTM')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.yticks(np.arange(60, 106, 5))
...
convergenceButton = Button(main, text="LSTM/BI-LSTM Convergence Graph", command=convergence_graph)
convergenceButton.place(x=960,y=600)
convergenceButton.config(font=font1)

graphButton = Button(main, text="All Model Accuracy Graph", command=graph)
